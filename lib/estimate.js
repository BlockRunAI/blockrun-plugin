...
// Savings vs the flagship model, for the "you saved $X" narrative.
// Only meaningful for image right now (flagship = gpt-image-2).
const IMAGE_FLAGSHIP = 0.08; // openai/gpt-image-2 base
function savings(toolName, input = {}) {
  const m = /blockrun_([a-z]+)$/.exec(toolName || "");
  if (!m || m[1] !== "image") return 0;
  const chosen = IMAGE[input.model] ?? IMAGE_DEFAULT;
